...
public class MyList<E> implements Cloneable, Visitable<E> {
  ...
  /**
   * Accept a Visitor.
   * Iterates over the elements of the list, asking the Visitor to visit them.
   * If the Visitor.visit() method returns false, no more elements will be visited.
   * If elements get removed from the list while accept() is in progress, unexpected
   * behaviour may occur.
   *
   * @author Philipp Weinbrenner
   */
  public void accept(Visitor<E> visitor) {
    for(MyEntry<E> current = begin.next; current != null; current = current.next)
      if (!visitor.visit(current.o))
        break;
  }
}
