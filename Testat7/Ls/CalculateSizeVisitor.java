...
//    System.err.println("visit " + p.toString() + " for " + path.toString());
    if(p.isDirectory()) {
...
//      System.out.println("TODO sub CalculateSizeVisitor for '" + p.toString() + "'");
      p.accept(subVisitor);
      sizes.putAll(subVisitor.getSizes());
//      System.out.println("DONE sub CalculateSizeVisitor for '" + p.toString() + "' with result " + subVisitor.getResult());
      sizes.put(p, subVisitor.getResult());
      sizes.put(path, Long.valueOf(sizes.get(path).longValue() + subVisitor.getResult()));
    } else {
//      System.err.println(" ... ordinary file of size " + p.length());
